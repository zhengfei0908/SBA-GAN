...
class BertEncoder(nn.Module):
    def __init__(self, embedding_dim=128):
        ...
        self.fc = nn.Linear(768, embedding_dim, bias = False)
    ...
    def forward(self, captions):
        ...
        words_embs, sent_emb = self.model(captions, segment_ids, mask_ids, output_all_encoded_layers=False)
...
class MAPPING_NET(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.z_dim, self.w_dim, bias=False),
    ...
    def forward(self, z_code):
        ...
        :return batch x cfg.GAN.W_DIM
        """
        w_code = self.fc(z_code)
        return w_code

    
class ADAIN_NORM(nn.Module):
    def __init__(self, ngf):
        super(ADAIN_NORM, self).__init__()
        
        w_dim = cfg.GAN.W_DIM
        self.norm = nn.InstanceNorm2d(ngf)
        self.style = nn.Linear(w_dim, ngf * 2)

    def forward(self, h_code, w_code):
        style = self.style(w_code).unsqueeze(2).unsqueeze(3)
        gamma, beta = style.chunk(2, 1)

        out = self.norm(h_code)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, w_code, word_embs, mask):
        """
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            h_code2, att1 = \
                self.h_net2(h_code1, c_code, w_code, word_embs, mask)
        ...
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3, att2 = \
                self.h_net3(h_code2, c_code, w_code, word_embs, mask)
